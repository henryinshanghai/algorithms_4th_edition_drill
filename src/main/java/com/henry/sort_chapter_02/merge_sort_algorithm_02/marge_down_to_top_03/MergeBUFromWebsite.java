 ...
 *  @author Kevin Wayne
 */
public class MergeBUFromWebsite {

    // This class should not be instantiated.
    private MergeBUFromWebsite() { }
    ...
    public static void sort(Comparable[] a) {
        ...
        for (int len = 1; len < n; len *= 2) { // 外循环执行的次数？ len以指数形式增长，当len接近于n时，数组也就排序完成了
            // 内循环的作用：以2*len作为单位，对整个数组划分得到的各个分组进行合并
            for (int lo = 0; lo < n-len; lo += len+len) {
                int mid  = lo+len-1;
                int hi = Math.min(lo+len+len-1, n-1); // 为什么是2*len？因为就是用2*len为单位对数组进行的划分
                merge(a, aux, lo, mid, hi); // 执行一次merge操作
            }
        }
        assert isSorted(a);
    }
