 ...
 */
public class Merge_round02_drill02 {
    ...
    private static void merge(Comparable[] a, int leftBar, int middle, int rightBar) {
        ...
        for (int cursor = leftBar; cursor <= rightBar; cursor++) { // here is <=
            aux[cursor] = a[cursor];
        }

        for (int cursor = leftBar; cursor <= rightBar; cursor++) { // "int cursor = 0; cursor < a.length; cursor++" is wrong
