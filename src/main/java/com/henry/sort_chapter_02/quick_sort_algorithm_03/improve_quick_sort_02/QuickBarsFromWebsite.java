...
public class QuickBarsFromWebsite {
    ...
    // quicksort the subarray from a[lo] to a[hi] 对a[lo ... hi]中的子数组进行快速排序
    private static void sort(double[] a, int lo, int hi) {
        // 改进1：改变基本情况为————数组很小（这时候直接交给快速排序解决就可以了）
        // cutoff to insertion sort 在递归调用的方法中，判断当前方法传入的参数hi、lo————从而判断数组是不是一个小数组
        int n = hi - lo + 1;
        if (n <= CUTOFF) { // 如果是一个小数组...
            insertionSort(a, lo, hi); // 使用插入排序对数组进行排序
//            show(a, lo, -1, -1, hi); 这是用来画柱形图的
            return;
        } // 这是一个新的基本条件

        // 改进2 选择更好的切分元素————子数组中某些个位置的中位数所对应的位置上的元素     原理：不清楚
        // 求取子数组的一些位置的中位数
        int m = median3(a, lo, lo + n/2, hi);
        // 交换两个位置上的元素
        exch(a, m, lo);

        int j = partition(a, lo, hi); // 切分的过程没有变化
        show(a, lo, j, j, hi);
        sort(a, lo, j-1);
        sort(a, j+1, hi);
    }
