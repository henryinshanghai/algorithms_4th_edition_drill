...
public class MaxPQFromWebsite<Key> implements Iterable<Key> {
     ...
     */
    public MaxPQFromWebsite(int initCapacity, Comparator<Key> comparator) {
     ...
     * @param  comparator the order in which to compare the keys
     */
    public MaxPQFromWebsite(Comparator<Key> comparator) {
        this(1, comparator);
    }
     ...
     */
    public MaxPQFromWebsite(Key[] keys) {
        n = keys.length;
    ...
    private class HeapIterator implements Iterator<Key> {
        ...
        // create a new pq
        private MaxPQFromWebsite<Key> copy;

        // add all items to copy of heap
        // takes linear time since already in heap order so no keys move
        public HeapIterator() {
            if (comparator == null) copy = new MaxPQFromWebsite<Key>(size());
            else                    copy = new MaxPQFromWebsite<Key>(size(), comparator);
            for (int i = 1; i <= n; i++)
                copy.insert(pq[i]);
        }
