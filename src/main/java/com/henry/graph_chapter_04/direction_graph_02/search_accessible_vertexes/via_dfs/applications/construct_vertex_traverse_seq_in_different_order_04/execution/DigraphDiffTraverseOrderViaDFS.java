package com.henry.graph_chapter_04.direction_graph_02.search_accessible_vertexes.via_dfs.applications.construct_vertex_traverse_seq_in_different_order_04.execution;
...
public class DigraphDiffTraverseOrderViaDFS {
    ...
    /**
     * 获取到 指定有向图的 深度优先遍历的结果序列
     *
     * @param digraph 指定的有向图
     */
    public DigraphDiffTraverseOrderViaDFS(Digraph digraph) {
        // 顶点在前序序列中的位置
        ...
        for (int currentVertex = 0; currentVertex < digraph.getVertexAmount(); currentVertex++)
            // 如果 当前节点 没有被标记，说明 其所属的子图 还没有被处理过，则：
            if (isNotMarked(currentVertex))
                // 以 当前节点 作为 起始节点，在图中 开始执行DFS
    ...
    /**
     * 获取到 指定的加权有向图的 深度优先遍历的结果序列
     *
     * @param digraph 指定的加权有向图
     */
...
//    }
    ...
    // 在 有向图G 中，以 指定节点 作为 起始节点 执行DFS，并 计算出其 前序节点序列 以及 后序节点序列
    private void markVertexesAndPickToSequenceViaDFS(Digraph digraph, int currentVertex) {
        // 标记结点
        vertexToIsMarked[currentVertex] = true;

        // 得到 前序遍历的序列 - 手段：在 递归调用 之前/当前调用中，把 当前结点 添加到 队列中
        System.out.println("~~~ 1 在图中，以 当前顶点" + currentVertex + " 作为 起始顶点的DFS过程 开始。同步构造 图节点的 前序序列 ~~~");
        ...
        // 得到 后序遍历的序列 - 手段：在 递归调用完成 之后，把 当前结点 添加到 队列中
        System.out.println("~~~ 2 在图中，以 当前顶点" + currentVertex + " 作为 起始顶点的DFS过程 结束。同步构造 图节点的 后序序列 ~~~");
...
//    }
    ...
    // key API*1: 获取到 指定顶点 在 前序遍历结果序列 中的位置
    ...
    public static void main(String[] args) {
        ...
        DigraphDiffTraverseOrderViaDFS markedDigraph = new DigraphDiffTraverseOrderViaDFS(digraph);
