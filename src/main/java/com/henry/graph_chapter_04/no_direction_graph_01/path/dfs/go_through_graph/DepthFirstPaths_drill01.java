...
public class DepthFirstPaths_drill01 {
    /* 根据具体任务进行成员变量的设置 */
    // 当前节点有没有被标记过
    ...
    // 构造方法的语法中不能够添加返回值类型
    public DepthFirstPaths_drill01(Graph G, int s) {
        // 初始状态都是未标记
    ...
    public static void main(String[] args) {
        ...
        // 创建 path对象
        DepthFirstPaths_drill01 pathIfExist = new DepthFirstPaths_drill01(graph, s);
        ...
        for (int v = 0; v < graph.V(); v++) {
            ...
            // 判断从起始节点s 到当前节点是否连通
            if (pathIfExist.hasPathTo(v)) {
                // 获取到 路径的可迭代对象(aka 栈对象) - 手段：foreach语法
                for (int x : pathIfExist.pathTo(v)) {
