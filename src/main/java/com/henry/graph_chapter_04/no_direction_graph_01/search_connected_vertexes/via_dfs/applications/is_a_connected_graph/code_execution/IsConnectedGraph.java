...
public class IsConnectedGraph {
    // 记录 顶点 -> 顶点是否被标记 手段：使用一个size 和 图中顶点数量相同 的boolean类型数组
    private boolean[] vertexToIsMarkedArr;
    // 记录 被标记的节点数量
    private int markedVertexAmount;


    // 构造器 - 作用#1：完成 成员变量的初始化
    ...
    // 如何验证 通过这种方式 能够遍历 图中所有的顶点与边？
    // 深度优先搜索 只能够找到 图G中所有 “与顶点s相连通”的顶点集合
    // 算法步骤：一边遍历，一边标记
