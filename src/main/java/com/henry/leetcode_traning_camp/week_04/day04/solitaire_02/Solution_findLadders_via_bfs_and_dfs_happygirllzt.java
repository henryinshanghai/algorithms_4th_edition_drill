...
public class Solution_findLadders_via_bfs_and_dfs_happygirllzt {
    ...
    private static void generateTheMapViaBFS(Set<String> startWordSetOnCurrentLevel,
        ...
        if (startWordSetOnCurrentLevel.size() == 0) {
            // 直接return 跳出方法
            return;
        }
        ...
        validMiddleWordSet.removeAll(startWordSetOnCurrentLevel);

        /* Ⅱ 准备一个循环 在当前level上对图进行扩展 */
        // 遍历startWordSet中的每一个单词，找到其 所有可用的转换结果 #1 添加到下一层中；#2 并构建图map；/或者直接返回
        for (String currentStartWordStr : startWordSetOnCurrentLevel) {
                ...
                for (char replacedLetter = 'a'; replacedLetter <= 'z'; replacedLetter++) {
                    ...
                    if (validMiddleWordSet.contains(currentReplacedResult)) {
                        ...
                        if (endWord.equals(currentReplacedResult)) {
                            ...
                            startWordSetOnNextLevel.add(currentReplacedResult);
                        }
        ...
        if (!isTransformFinished) {
            ...
            generateTheMapViaBFS(startWordSetOnNextLevel, // 🐖 此时 startWordSet 的值就是 startWordSetOnNextLevel了 - 也就是图中 下一个level的所有结点
                    endWord,
                    wordToItsChangedMiddleWordMap,
                    validMiddleWordSet);
        }
        // 如果finish为true，则：图已经创建完成,方法结束
    }
}