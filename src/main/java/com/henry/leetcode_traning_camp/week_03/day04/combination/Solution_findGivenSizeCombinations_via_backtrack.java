...
public class Solution_findGivenSizeCombinations_via_backtrack {
    public static void main(String[] args) {
        ...
        // 获取到 所有的组合的列表
        List<List<Integer>> combinationList = getCombinationFrom(availableMaxItem, combinationItemAmount);
        // 逐个打印 所有组合
    ...
    private static List<List<Integer>> getCombinationFrom(int availableMaxItem, int expectedCombinationItemAmount) {
        // #0 健壮性代码 对传入的参数进行判断
        if (expectedCombinationItemAmount <= 0 || expectedCombinationItemAmount > availableMaxItem) {
            return combinationList;
        }

        // #1 利用回溯算法 来 得到“所有可能的组合的集合”
        constructGivenSizeCombinationFrom(availableMaxItem, expectedCombinationItemAmount, 1);
    ...
    private static void constructGivenSizeCombinationFrom(int availableMaxItem, int expectedCombinationItemAmount,
        ...
        for (int currentItem = dynamicStartAnchorItem; currentItem <= availableMaxItem; currentItem++) {
            ...
            // #2-2 递归地 从剩余的(N-1)个元素中，挑选出剩下的(K-1)个元素  aka 补齐集合中的剩余元素 👇
            // 子问题：在包含有(N-1)个自然数的序列中，挑选出(K-1)个元素 构成“大小为(K-1)的组合”
            // 子问题的解 就是原始问题的解的一部分
            constructGivenSizeCombinationFrom(availableMaxItem, expectedCombinationItemAmount, currentItem + 1); // 注意这里的代码时i + 1,而不是anchorItem + 1
