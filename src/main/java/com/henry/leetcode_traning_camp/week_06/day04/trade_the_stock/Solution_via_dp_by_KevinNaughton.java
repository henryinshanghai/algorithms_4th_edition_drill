...
public class Solution_via_dp_by_KevinNaughton {
    public static void main(String[] args) {
        int[] currentDayToItsStockValueArr = {7, 1, 5, 3, 6, 4}; // 5
//        int[] currentDayToItsStockValueArr = {7, 6, 4, 3, 1}; // 0

        int maxProfit = maxProfitCouldGetFrom(currentDayToItsStockValueArr);

        System.out.println("在股票价格序列: " + Arrays.toString(currentDayToItsStockValueArr) + "中" +
                "一次交易所能赚取的最大利润为：" + maxProfit);
    }

    private static int maxProfitCouldGetFrom(int[] currentDayToItsStockValueArr) {
        ...
        for (int currentDay = 0; currentDay < currentDayToItsStockValueArr.length; currentDay++) {
            ...
            if (currentDayStockValue < minStockValueSoFar) {
                // 使用它 来 更新“最低股价”的变量
                minStockValueSoFar = currentDayStockValue;
            } else { // 如果 当前股价比起“当前最低股价”更高，说明 可以进行交易来获利，则：
                // 比较 “在当前天数进行交易”所能的获利 与 “当前最大获利”，取两者中的最大值 作为“最大利润‘
                maxProfit = Math.max(
                        maxProfit, // 当前最大获利
                        currentDayStockValue - minStockValueSoFar); // “在当前天数进行交易”所能的获利
            }
        }
