...
public class Solution_buyAndSellWithKTransaction_04_categorize_by_jayatiTiwari_drill02 {
    ...
    private static int maxProfit(int[] stockValuesThroughDays, int k) {
        ...
        // 1 准备数组 moneyInHandAfterBuy  存储执行第(i+1)次buy操作后手上的现金
        int[] moneyInHandAfterBuy = new int[k];
        Arrays.fill(moneyInHandAfterBuy, Integer.MIN_VALUE);

        // 2 准备数组 moneyInHandAfterSell  存储执行第(i+1)次sell操作后手上的现金
        ...
        for (int i = 0; i < len; i++) {
            for (int j = 0; j < k; j++) {
                ...
                moneyInHandAfterBuy[j] = Math.max(
                        moneyInHandAfterBuy[j],
                        (j == 0)
                ...
                moneyInHandAfterSell[j] = Math.max(
                        moneyInHandAfterSell[j],
                        moneyInHandAfterBuy[j] + stockValuesThroughDays[i]
                );
            }
        }
