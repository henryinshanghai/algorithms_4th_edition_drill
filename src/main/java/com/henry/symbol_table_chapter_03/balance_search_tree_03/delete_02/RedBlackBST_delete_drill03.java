...
public class RedBlackBST_delete_drill03<Key extends Comparable<Key>, Value> {
    private Node root;
    ...
    private Node delete(Node x, Key key) {
        ...
        if (key.compareTo(x.key) < 0) {
            if (!isRed(x.left) && !isRed(x.left.left)) { // 当前节点的左子节点（？）是2-节点
                x = moveRedToLeft(x);
            }
            ...
            if (key.compareTo(x.key) == 0) {
                ...
                // phrase 3 delete from right subtree
                x.right = delete(x.right, key);
            }
        }

        // after recursive call, re-balance the tree's color
        return rebalance(x);
    }
