...
/******************************************************************************
 *  Compilation:  javac WeightedQuickUnionUF_03.java
 *  Execution:  java WeightedQuickUnionUF_03 < input.txt
...
/**
 *  The {@code WeightedQuickUnionUF_03} class represents a <em>union–find data type</em>
 ...
 *  @author Kevin Wayne
 */
public class WeightedQuickUnionUF_03 {
    private int[] parent;   // parent[i] = parent of i
    // 不是根节点的节点，这个值为曾经的分量中的节点数  注：一旦这个位置的节点不再是根节点，这里存储的值就不会再被使用了
     ...
     */
    public WeightedQuickUnionUF_03(int n) {
        count = n;
        parent = new int[n];
        size = new int[n];
        for (int i = 0; i < n; i++) {
            parent[i] = i;
            size[i] = 1; // 最开始时，每个节点都是一个分量 所以分量中的节点数为1
        }
    }
    ...
    public void union(int p, int q) {
        ...
        // make smaller root point to larger one    使较小的树的根节点连接到较大的树的根节点上
        if (size[rootP] < size[rootQ]) { // 找到较小的树的根节点... 因为需要更新它的值
            // 更新较小数的根节点的值 = 较大树根节点的索引/值
            parent[rootP] = rootQ;
            // 更新较大树根节点所代表的分量中的节点数 = 旧值 + 较小树中的节点数
    ...
    public static void main(String[] args) {
        int n = StdIn.readInt();
        WeightedQuickUnionUF_03 uf = new WeightedQuickUnionUF_03(n);
        while (!StdIn.isEmpty()) {
            int p = StdIn.readInt();
            int q = StdIn.readInt();
